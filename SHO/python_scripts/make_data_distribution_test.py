...
def generate_IC(n=max_n,random=False):
    ...
    if random:
        ...
        IC[random_index] = np.random.rand() + 1/3
        # IC[1:] = np.random.rand(n-1)/2
        IC[0] = 1
        IC = IC/np.sqrt(np.sum(IC**2))
        return IC
    else:
        IC[0] = 1
        IC[1] = 1/2 +  np.random.rand()/1
        IC[2] = 1/2 +  np.random.rand()/2
        IC[3] = 1/2 +  np.random.rand()/3
        IC[4] = 1/2  + np.random.rand()/4
        IC = IC/np.sqrt(np.sum(IC**2))
        return IC
...
if new_training:
    ...
    Nomega = 100000
    omega_values = np.random.uniform(2,8,Nomega)
    
    fixed_points = False
    if fixed_points :
        Nomega = 7
        omega_values = np.array([2,3,4,5,6,7,8])
    omega_index = 0 
    for omega_value in omega_values:
        ...
        for k in range(0,files_per_frequency):
            ...
            if fixed_points:
                np.save('../data/omega_distribution/SpacedTest/sho_'+str(omega_index)+'_'+str(k)+'.npy',data.view(float))
            else :
                np.save('../data/omega_distribution/DedicatedTest2/sho_'+str(omega_index)+'_'+str(k)+'.npy',data.view(float))
        omega_index+=1
