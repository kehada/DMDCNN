...
live_frac_values = np.random.uniform(0.2,0.8,N_live_frac)
...
for live_frac in live_frac_values:
    ...
    for i in range(0,files_per_live_frac):
        ...
        for j in range(0,snapshots_per_sim):
            ...
            np.save('E:/Research/DMDCNN_suplement/Isothermal/data/rank'+str(rank)+'/dedicated_test/'+str(filename+start),np.array(D).view(float))
            print('Saved file:E:/Research/DMDCNN_suplement/Isothermal/data/rank'+str(rank)+'/dedicated_test/'+str(filename+start))
            filename+=1            
        file_index += 1
    live_frac_index += 1
